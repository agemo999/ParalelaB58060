...
void find_nearest ( int mind[TAMANO], bool connected[TAMANO], int *d, int *v,int msize, int mrank ) {
//
//  Purpose:
//
...
int *ppc_distance ( int matrix[TAMANO][TAMANO], int start_node ,int msize, int mrank) {
...
//
    find_nearest ( mind, connected, &md, &mv ,msize,mrank);
...
void init ( int matrix[TAMANO][TAMANO],int msize, int mrank ) {
...
//
  ...
  if(TAMANo%mysize==0){
    ...
    distrib=TAMANO/mysize;

  }else{
    distrib=TAMANO/mysize+1;
...
//
  mind = ppc_distance( matrix, i , mysize, myrank);
//
//  Print the results.
//
    cout << "\n";
